        ...
        if os.path.exists(fullpath) == True:
            ...
            if (file_count == 4 and shp_file != '' and file_type == 'SHP' and prj_file != ''):
                ...
                shell = 'shp2pgsql -I -s 4326 ' + shp_path + ' public.' + new_table_name + \
                        ' | psql -U ' + default_db['USER'] + ' -d ' + default_db['NAME'] + ' -h localhost'
...
def layer_data(request, layer_id=None, groupId=None, *args, **kwargs):
    ...
    shell = 'ogr2ogr -f GeoJSON -t_srs EPSG:4326 ' + geojsonPath + \
        ' "PG:dbname=' + default_db['NAME'] + ' user=' + default_db['USER'] + ' host=localhost" -sql "select * from public.' + table_name + '"'
    subprocess.run(shell, shell=True)
        ...
        if len(draw_layer) > 0:
            for drawLayer in draw_layer:
                if drawLayer.layerdraw_status == 1:
                    lastTime = drawLayer.updated_at.replace(tzinfo=None)
                    print(lastTime)
                    diff = nowTime - lastTime
                    day = diff.days
                    seconds = diff.seconds
...
def get_lastest_layer_data(request, groupId=None, *args, **kwargs):
    ...
    if len(layer_draw) > 0:
        lastTime = layer_draw[0].updated_at.replace(tzinfo=None)
...
def ds(request, province=None, district=None, *args, **kwargs):
    main_server = settings.AAIB_URL
    aaib_token = request.session['access_token']
    url = str(main_server) + 'api/api/ds&province=' + str(province) + '&district=' + str(
        district) + '&access-token=' + str(aaib_token)
    http = urllib3.PoolManager()
    r = http.request('GET', url)
    result = json.loads(r.data.decode('utf-8'))
    response = {'ds': result}
...
def gn(request, province=None, district=None, ds=None, *args, **kwargs):
    main_server = settings.AAIB_URL
    aaib_token = request.session['access_token']
    url = str(main_server) + 'api/api/gn&province=' + str(province) + '&district=' + str(district) + '&ds=' + str(
        ds) + '&access-token=' + str(aaib_token)
    http = urllib3.PoolManager()
    r = http.request('GET', url)
    result = json.loads(r.data.decode('utf-8'))
    response = {'gn': result}
...
def search_public_layers_list(request, layer_status_type=None, gid=None, **kwargs):
    ...
    if (layer_status_type == 'public'):
        ...
        if (layer_status_type == 'public'):
            ...
            if (search != None):
                map_layer = Layerfiles.objects.filter(layer_name__icontains=search).filter(
                    layer_status='public').order_by('layer_name')
        ...
        if len(map_layer) > 0:
            for mapLayer in map_layer:
                if (mapLayer.layerfiles_status == 1 and mapLayer.layer_type != 'KML'):
                    ...
                    lists_tuples.append({'layer_name': mapLayer.layer_name, 'layerID': mapLayer.pk, 'layerType': 'map',
                                         'description': mapLayer.layer_descri, 'fileType': mapLayer.layer_type,
                                         'fileName': layerfileName, 'userType': 'user', 'group_id': ''})
...
def aaib_layer_data(request, layer_id=None, *args, **kwargs):
    ...
    shell = 'ogr2ogr -f GeoJSON -t_srs EPSG:4326 ' + geojsonPath + ' "PG:dbname=' + default_db['NAME'] + ' user=' + \
            default_db['USER'] + ' password=' + default_db[
                'PASSWORD'] + '" -sql "select * from public.' + table_name + '"'
    subprocess.call(shell, shell=True)
