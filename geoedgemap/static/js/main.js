...
function layer_to_map(layer_id, layer_type, layer_file) {
  ...
  if (layer_id != '' || layer_type != '') {
    if (layer_type == 'SHP') {
      var getGeojson = $.ajax({
        url:  '/map/layer_data/' + layer_id + '/' + gourpID + '/',
        dataType: 'json',
...
function geojsons_to_map(layer_id, group_id) {
  var get_data = $.ajax({
    url:  '/map/layersdata/' + layer_id + '/' + gourpID + '/',
    type: 'GET',
    dataType: 'json',
    //                  success: ,
    error: function (xhr) {
...
function map_count_fun(groupID = '') {
  ...
  if (groupID > 0) {
    gourpID = groupID;
    url =  '/map/map_group_count/' + groupID + '/';
  } else {
    url =  '/map/map_count/';
  }
...
function lastest_layer() {
  $.ajax({
    url:  '/map/lastest_layer_data/' + gourpID + '/',
    dataType: 'json',
    success: function (data) {
      if (data.status == 1) {
        ...
        if (lastestNews[3] == 'layer') {
          ...
          if (groupID > 0) {
            ...
            a.href =  '/map/' + lastestNews[1] + '/layer';
          }
        }
      } else {
        $('#map_view').hide();
