...
/* [START] - Post Layer */
    $("#layer_submit").click(function (){
        ...
        if(layerStatus == true){
            ...
            post_data = {'layer_name':layer_name, 'layer_status':layer_status, 'layer_descri':layer_descri};
            $.ajax({
...
function init(){
    map = new L.Map('map', {center: new L.LatLng(0, 0), zoom: 2, minZoom: 2, zoomControl:false});
            ...
            if(layer_type == 'SHP'){
                ...
                var getGeojson = $.ajax({
                    ...
                    $.when(getGeojson).done(function() {
                        ...
                        geojsonMap = L.geoJson(to_json, {
                                    style: function(feature){

                                        var fillcolor = (!feature.properties['_layerFillColor'])? random_color : feature.properties['_layerFillColor'];
                                ...
                                pointToLayer: function(feature, latlng) {
                                    ...
                                    if(geometry['type'] == 'Point'){
                                        ...
                                        if(_layerRadius != null){
                                            ...
                                            var circle = L.circle([latlng.lat, latlng.lng],{radius: _layerRadius, color:color,weight:weight, opacity:opacity,fillColor: fillcolor,fillOpacity:fillOpacity});
                                             return circle;
          ...
          success: function(data){
            if(data.status == 1){
                ...
                for (i=0;i<lastestMapsSort.length; i++){
                    ...
                    var list = '<li style="margin-left:10px;" class="list-group-item">'
                            +'<a href="'+lastestMapsSort[i][4]+'" target="_blank">'+lastestMapsSort[i][2]+'</a>'
          ...
          success: function(data){
            if(data.status == 1){
                ...
                for (i=0;i<lastestNewsSort.length; i++){
                    ...
                    var list = '<li style="margin-left:10px;" class="list-group-item">'
                            +'<a href="'+lastestNewsSort[i][5]+'" target="_blank">'+lastestNewsSort[i][2]+'</a>'
                            +'<small class="block text-muted">'
                            +'<i class="fa fa-clock-o"></i> '+time
                            +'</small>'
                            +'</li>';
            ...
            if(data.status == 1){
                ...
                $('#recent_mheading').html(layer_tile);
                    ...
                    if(lastestNews[3] == 'map'){
                        ...
                        if(groupID>0){
                            a.href = subdivision+"/map/group/"+groupID+"/"+lastestNews[7]+"/map";
                        }else{
                            a.href = subdivision+"/map/"+lastestNews[1]+"/map";
                        }
                    }

                    if(lastestNews[3] == 'layer'){ // GeoJson Data
                        ...
                        if(groupID>0){
                            a.href = subdivision+"/map/group/"+groupID+"/"+lastestNews[1]+"/layer";
                        }else{
...
function click_page(sub_proj,groupId = ''){
    ...
    var table = '<table id="'+sub_proj+'" style="width:99%;min-height:150px;position:relative;left:400px;"  title="Map Manager" singleSelect="true" idField="itemid" fitColumns="true"  rownumbers="true" pagination="true"></table>'
    $('#page_table').html(table);

    /*Button for New Map or Group*/
    if(sub_project_page == 'my_map' || sub_project_page == 'my_layer' || sub_project_page == 'all_map' || sub_project_page == 'all_layer'){
        $('#new_map_button').show();
        $('#new_group_button').hide();
        $('#new_user_button').hide();
        $('#new_group_member_button').hide();
    }else if(sub_project_page == 'my_group' || sub_project_page == 'all_group'){
...
function pageDataUrl(){
    if(sub_project_page == 'my_map'){
        page_data_url = server_url+subdivision+'/map/my_map_data/';
    }else if(sub_project_page == 'my_layer'){
        page_data_url = server_url+subdivision+'/map/my_layer_data/';
    }else if(sub_project_page == 'all_map'){
        page_data_url = server_url+subdivision+'/map/all_map_data/';
    }else if(sub_project_page == 'all_layer'){
        page_data_url = server_url+subdivision+'/map/all_layer_data/';
    }else if(sub_project_page == 'my_group'){
        page_data_url = server_url+subdivision+'/map/my_group/';
    }else if(sub_project_page == 'all_group'){
        page_data_url = server_url+subdivision+'/map/all_group/';
    }else if(sub_project_page == 'user_manage'){
        page_data_url = server_url+subdivision+'/user_data/';
    }else if(sub_project_page == 'group_map'){
        page_data_url = server_url+subdivision+'/map/group_maps/'+groupID+'/';
    }else if(sub_project_page == 'group_layer'){
        page_data_url = server_url+subdivision+'/map/group_layers/'+groupID+'/';
    }else if(sub_project_page == 'group_member'){
        page_data_url = server_url+subdivision+'/map/group_members/'+groupID+'/';
    }else{
        page_data_url = '';
    }
...
function column_gen(){
    if(sub_project_page == 'my_map' || sub_project_page == 'all_map'){
        pageColumns = [[
					...
					{field:'description',title:'Description',width:200,editor:{type:'text'}},
					{field:'created_date',title:'Created Date',width:80, formatter:function(value,row,index){
					...
					{field:'action',title:'Action',width:30,align:'center',
						formatter:function(value,row,index){
							if (row.editing){
								var s = '<a href="javascript:void(0)" onclick="saverow(this)"><i class="fa fa-check" style="color:#01A9DB"></i></a> ';
								var c = '<a href="javascript:void(0)" onclick="cancelrow(this)"><i class="fa fa-times" style="color:#01A9DB"></a>';
								return s+c;
							} else {
							    var f = '<a href="'+server_url+subdivision+'/map/'+row.id+'/map" target="_blank"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-external-link"></i></a> ';
								var e = '<a href="javascript:void(0)" onclick="editrow(this)"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-pencil"></i></a> ';
								var d = '<a href="javascript:void(0)" onclick="deleterow(this,'+row.id+')"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-trash-o"></i></a>';
								return f+e+d;
							}
						}
					}
				]];
    }else if(sub_project_page == 'my_layer' || sub_project_page == 'all_layer'){
					...
					{field:'action',title:'Action',width:30,align:'center',
						formatter:function(value,row,index){
							if (row.editing){
							    ...
							    var f = '<a href="'+server_url+subdivision+'/map/'+row.id+'/layer" target="_blank"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-external-link"></i></a> ';
								var e = '<a href="javascript:void(0)" onclick="editrow(this)"><i style="color:#01A9DB" class="fa fa-pencil"></i></a> ';
								var d = '<a href="javascript:void(0)" onclick="deleterow(this,'+row.id+')"><i style="color:#01A9DB" class="fa fa-trash-o"></i></a>';
								return f+e+d;
							}
						}
					}
        ...
        pageColumns = [[
					{field:'group_name',title:'Name',width:100,
						formatter:function(value,row){
							return row.name || value;
						},editor:{
							type:'text'
					...
					{field:'action',title:'Action',width:30,align:'center',
						formatter:function(value,row,index){
							if (row.editing){
							    ...
							    var f = '<a href="'+server_url+subdivision+'/group/'+row.id+'/" target="_blank"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-external-link"></i></a> ';
                                var e ='', d='';
							    if(userAccess != false){
                                    e = '<a href="javascript:void(0)" onclick="editrow(this)"><i style="color:#01A9DB" class="fa fa-pencil"></i></a> ';
								    d = '<a href="javascript:void(0)" onclick="deleterow(this,'+row.id+')"><i style="color:#01A9DB" class="fa fa-trash-o"></i></a>';
        ...
        pageColumns = [[
					...
					{field:'last_name',title:'Last Name',width:70,editor:{type:'text'}},
					...
					{field:'action',title:'Action',width:30,align:'center',
						formatter:function(value,row,index){
							if (row.editing){
							    ...
							    var f = '<a href="'+server_url+subdivision+'/map/group/'+groupID+'/'+row.id+'/map" target="_blank"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-external-link"></i></a> ';
								var e ='', d='';
							    if(userAccess != false){
								    e = '<a href="javascript:void(0)" onclick="editrow(this)"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-pencil"></i></a> ';
								    d = '<a href="javascript:void(0)" onclick="deleterow(this,'+row.id+')"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-trash-o"></i></a>';
								}
								return f+e+d;
							}
						}
					...
					{field:'action',title:'Action',width:30,align:'center',
						formatter:function(value,row,index){
							if (row.editing){
							    ...
							    var f = '<a href="'+server_url+subdivision+'/map/group/'+groupID+'/'+row.id+'/layer" target="_blank"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-external-link"></i></a> ';
								var e ='', d='';
							    if(userAccess != false){
								    e = '<a href="javascript:void(0)" onclick="editrow(this)"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-pencil"></i></a> ';
								    d = '<a href="javascript:void(0)" onclick="deleterow(this,'+row.id+')"><i style="color:#01A9DB; padding-left:2px;" class="fa fa-trash-o"></i></a>';
        ...
        pageColumns = [[
					...
					{field:'superuserCk',title:'Admin', width:12, align:'center',
                        formatter:function(value,row){
                                ...
                                if(superuser == 1){
                                    var d = '<input type="checkbox" checked onclick="superuser_update(this,'+row.id+','+groupID+')" '+checkboxEnable+' '+uAccess+'>';
                                }else{
                                    var d = '<input type="checkbox" onclick="superuser_update(this,'+row.id+','+groupID+')" '+uAccess+'>';
                                }
                                return d;
                            }
					},
...
function superuser_update(target, id, groupID){
    ...
    $.ajax({
          url: subdivision+'/map/superuser_update/',
          data: post_data,
          type: 'POST',
          dataType: 'json',
          success: function(data){
...
function editData(data){
    ...
    var post_data = data, url;

    if(sub_project_page != 'user_manage'){
        url = subdivision+'/map/data_update/';
    }else{
        url = subdivision+'/user_data_update/';
    }
    $.ajax({
              url: url,
              data: post_data,
...
/* Add Member*/
 function newMemner(){
    ...
    if(gourp_id !=''){
      $.ajax({
          url: "/geoedgepro/member_list/"+gourp_id+"/",
          //type: 'POST',
          dataType: 'json',
          success: function(data){
              var userList = ''
              ...
              if(users.length > 0){
                for(u=0;u<users.length;u++){
                    userList += '<option value="'+users[u].id+'">'+users[u].name+'</option>';
                }
...
function saveMember(){
    ...
    if(!gourp_id){
        ...
        $('#fm_member').form('submit',{
            url: subdivision+'/map/add_member/',
            onSubmit: function(){
                return $(this).form('validate');
            },
            success: function(result){
	...
	$('#group_profile_button').click(function(e){
		...
		if(error_free == false){
            ...
            $('#group_profile_form').form('submit',{
                url: subdivision+'/map/group_update/',
                onSubmit: function(){
                    return $(this).form('validate');
                },
                success: function(result){
